import type { INodeProperties } from 'n8n-workflow';

export const bluetoothClientsDescription: INodeProperties[] = [
                {
			"displayName": "Operation",
			"name": "operation",
			"type": "options",
			"noDataExpression": true,
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					]
				}
			},
			"options": [
				{
					"name": "Get Network Bluetooth Clients",
					"value": "Get Network Bluetooth Clients",
					"action": "List the Bluetooth clients seen by APs in this network",
					"description": "List the Bluetooth clients seen by APs in this network",
					"routing": {
						"request": {
							"method": "GET",
							"url": "=/networks/{{$parameter[\"networkId\"]}}/bluetoothClients"
						}
					}
				},
				{
					"name": "Get Network Bluetooth Client",
					"value": "Get Network Bluetooth Client",
					"action": "Return a Bluetooth client",
					"description": "Return a Bluetooth client. Bluetooth clients can be identified by their ID or their MAC.",
					"routing": {
						"request": {
							"method": "GET",
							"url": "=/networks/{{$parameter[\"networkId\"]}}/bluetoothClients/{{$parameter[\"bluetoothClientId\"]}}"
						}
					}
				}
			],
			"default": ""
		},
		{
			"displayName": "GET /networks/{networkId}/bluetoothClients",
			"name": "operation",
			"type": "notice",
			"typeOptions": {
				"theme": "info"
			},
			"default": "",
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "Network ID",
			"name": "networkId",
			"required": true,
			"default": "",
			"type": "string",
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "T 0",
			"name": "t0",
			"description": "The beginning of the timespan for the data. The maximum lookback period is 7 days from today.",
			"default": "",
			"type": "string",
			"routing": {
				"send": {
					"type": "query",
					"property": "t0",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "Timespan",
			"name": "timespan",
			"description": "The timespan for which the information will be fetched. If specifying timespan, do not specify parameter t0. The value must be in seconds and be less than or equal to 7 days. The default is 1 day.",
			"default": 0,
			"type": "number",
			"routing": {
				"send": {
					"type": "query",
					"property": "timespan",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "Per Page",
			"name": "perPage",
			"description": "The number of entries per page returned. Acceptable range is 5 - 1000. Default is 10.",
			"default": 0,
			"type": "number",
			"routing": {
				"send": {
					"type": "query",
					"property": "perPage",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "Starting After",
			"name": "startingAfter",
			"description": "A token used by the server to indicate the start of the page. Often this is a timestamp or an ID but it is not limited to those. This parameter should not be defined by client applications. The link for the first, last, prev, or next page in the HTTP Link header should define it.",
			"default": "",
			"type": "string",
			"routing": {
				"send": {
					"type": "query",
					"property": "startingAfter",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "Ending Before",
			"name": "endingBefore",
			"description": "A token used by the server to indicate the end of the page. Often this is a timestamp or an ID but it is not limited to those. This parameter should not be defined by client applications. The link for the first, last, prev, or next page in the HTTP Link header should define it.",
			"default": "",
			"type": "string",
			"routing": {
				"send": {
					"type": "query",
					"property": "endingBefore",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "Include Connectivity History",
			"name": "includeConnectivityHistory",
			"description": "Include the connectivity history for this client",
			"default": true,
			"type": "boolean",
			"routing": {
				"send": {
					"type": "query",
					"property": "includeConnectivityHistory",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "X Cisco Meraki API Key (Header)",
			"name": "security_meraki_api_key",
			"type": "string",
			"default": "",
			"description": "API key for meraki_api_key (header: X-Cisco-Meraki-API-Key)",
			"required": false,
			"routing": {
				"request": {
					"headers": {
						"X-Cisco-Meraki-API-Key": "={{ $value }}"
					}
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Clients"
					]
				}
			}
		},
		{
			"displayName": "GET /networks/{networkId}/bluetoothClients/{bluetoothClientId}",
			"name": "operation",
			"type": "notice",
			"typeOptions": {
				"theme": "info"
			},
			"default": "",
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Client"
					]
				}
			}
		},
		{
			"displayName": "Network ID",
			"name": "networkId",
			"required": true,
			"default": "",
			"type": "string",
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Client"
					]
				}
			}
		},
		{
			"displayName": "Bluetooth Client ID",
			"name": "bluetoothClientId",
			"required": true,
			"default": "",
			"type": "string",
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Client"
					]
				}
			}
		},
		{
			"displayName": "Include Connectivity History",
			"name": "includeConnectivityHistory",
			"description": "Include the connectivity history for this client",
			"default": true,
			"type": "boolean",
			"routing": {
				"send": {
					"type": "query",
					"property": "includeConnectivityHistory",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Client"
					]
				}
			}
		},
		{
			"displayName": "Connectivity History Timespan",
			"name": "connectivityHistoryTimespan",
			"description": "The timespan, in seconds, for the connectivityHistory data. By default 1 day, 86400, will be used.",
			"default": 0,
			"type": "number",
			"routing": {
				"send": {
					"type": "query",
					"property": "connectivityHistoryTimespan",
					"value": "={{ $value }}",
					"propertyInDotNotation": false
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Client"
					]
				}
			}
		},
		{
			"displayName": "X Cisco Meraki API Key (Header)",
			"name": "security_meraki_api_key",
			"type": "string",
			"default": "",
			"description": "API key for meraki_api_key (header: X-Cisco-Meraki-API-Key)",
			"required": false,
			"routing": {
				"request": {
					"headers": {
						"X-Cisco-Meraki-API-Key": "={{ $value }}"
					}
				}
			},
			"displayOptions": {
				"show": {
					"resource": [
						"Bluetooth Clients"
					],
					"operation": [
						"Get Network Bluetooth Client"
					]
				}
			}
		},
];
